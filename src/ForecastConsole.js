...
const ForecastConsole = ({ db, projects, assignments, currentTheme, appId }) => {
    ...
    const { forecastData, assignedData } = useMemo(() => {
        if (!weeklyHoursData.length && !assignments.length) return { forecastData: [], assignedData: [] };

        const filteredWeeklyHours = weeklyHoursData.filter(data => activeStatuses.includes(data.projectStatus));

        // Process forecast data (demand)
        const hoursByTrade = {};
        filteredWeeklyHours.forEach(data => {
            if (!hoursByTrade[data.trade]) {
                hoursByTrade[data.trade] = new Array(weekDates.length).fill(0);
            }
            weekDates.forEach((week, i) => {
                hoursByTrade[data.trade][i] += data.hours[week] || 0;
            });
        });
        const forecastData = Object.entries(hoursByTrade).map(([trade, hoursArray]) => ({
            trade,
        ...
        return { forecastData, assignedData };
    ...
    useEffect(() => {
        ...
        // --- NEW: Add 40-hour separation lines ---
    ...
    }, [forecastData, assignedData, boundedHeight, boundedWidth, margin.left, margin.top, weekDates, currentTheme, loading]);
